...
4. **Subjects** — left-rail sidebar of the user's folders. Selecting a subject opens a per-folder detail page that lists every item in that folder using the same card the Feed uses. The per-item page renders the item's `.md` with `react-markdown` + GFM via the `getMarkdown(item)` helper, which today reads the v1 Storage seam at `summaries/{uid}/{slug}/{emailId}.md` and is the single switch point for the future Drive backend. The watcher emits `.md` blobs to that path and sets `markdownStoragePath` on the item doc, so newly-processed items render real markdown; items processed before this rollout still show the empty state until they're re-summarized. The sidebar renders virtual groups as collapsible parents above ungrouped subjects (collapse state in `localStorage`); per-group Ask scoping ships with the Ask UI PR. The "Ask this subject" Sources + scoped chat split (NotebookLM mode) is still deferred and ships in a dedicated PR.
5. **Settings** — five sections: Inbox (Gmail connection), What to watch (sender/domain list), Where to save (Drive folder), Notifications, Privacy & data (export, delete account). Subject groups also lives here.
   - **Current state:** the **Watched senders**, **Notifications**, **Subject groups**, and **Privacy & data** editors are live. Watched senders writes `priorityWatchSenders` on `users/{uid}/config/main`; Notifications writes `digestEnabled`, `telegramEnabled`, and `telegramChatId` on the same doc (rules allow that exact allowlist). Subject groups writes to `users/{uid}/groups/{groupId}` (create / rename / set members / delete) — the "subject in zero or one groups" invariant is enforced client-side via a batched cross-group write (Firestore rules can't express it). Privacy & data: **Export** reads identity + config + folders + items via the client SDK and downloads a JSON file (no Cloud Function needed); **Delete account** calls the `deleteAccount` Cloud Function (admin SDK) which recursive-deletes Firestore under `users/{uid}/**` (groups included), deletes Storage prefixes `summaries/{uid}/` and `pdfs/{uid}/`, and finally removes the Auth user. The remaining two sections (Inbox, Where to save) ship in dedicated PRs. The page is reachable from the sidebar's Settings link.
