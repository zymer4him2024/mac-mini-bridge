...
3. **Feed** — chronological list of processed emails, rendered inside the persistent app shell (left sidebar + main pane). Each card matches the Telegram bot card layout: mailbox emoji + sender, subject, two-bullet summary, priority badge, timestamp, attached `.md` filename and size.
4. **Subjects** — left-rail sidebar of the user's folders. Selecting a subject opens a per-folder detail page that lists every item in that folder using the same card the Feed uses. Virtual groups, the per-subject markdown reader, and the "Ask this subject" Sources + scoped chat split (NotebookLM mode) are still deferred and ship in dedicated PRs.
5. **Settings** — five sections: Inbox (Gmail connection), What to watch (sender/domain list), Where to save (Drive folder), Notifications, Privacy & data (export, delete account).
   - **Current state:** the **Watched senders** and **Notifications** editors are live. Watched senders writes `priorityWatchSenders` on `users/{uid}/config/main`; Notifications writes `digestEnabled`, `telegramEnabled`, and `telegramChatId` on the same doc (rules allow that exact allowlist). The other three sections (Inbox, Where to save, Privacy & data) ship in dedicated PRs. The page is reachable from the sidebar's Settings link.
